...
class RTree(object):
    ...
    def query_rect_solutions(self, r):
        return self.cursor.query_rect_solutions(r)
    def query_point_solutions(self, p):
        return self.cursor.query_point_solutions(p)

    def _add_children(self, inpNode, inddict):
        idx = inpNode.index
        inddict[idx] = []
        if not inpNode.is_leaf():
            for c in inpNode.children():
                inddict[idx].append(c.index)

    def _get_node_dict(self, inpNode, inddict):
        self._add_children(inpNode, inddict)
        if not inpNode.is_leaf():
            for c in root.children():
                self._get_node_dict(c, inddict)

    def _buildidx_recurse(self, inpNode, inddict):
        idx = inpNode.index
        inddict[idx] = []
        if not inpNode.is_leaf():
            for c in inpNode.children():
                inddict[idx].append(c.index)
                self._buildidx_recurse(c, inddict)

    def build_dict_index(self):
        inddict = {}
        self._buildidx_recurse(self.cursor, inddict)
        return inddict
...
class _NodeCursor(object):
    ...
    def walk_solutions(self, predicate, res, curRes):
        if (predicate(self, self.leaf_obj())):
            curRes.append(self.index)
            if self.is_leaf():
                res.append(tuple(curRes))
            else:
                for c in self.children():
                    c.walk_solutions(predicate, res, curRes)
    ...
    def query_rect_solutions(self, r):
        ...
        res = []
        curRes = []
        self.walk_solutions(p, res, curRes)
        return res
    ...
    def query_point_solutions(self, point):
        """ Return things that intersect with 'r'. """
        def p(o,x): return o.rect.does_containpoint(point)
        res = []
        curRes = []
