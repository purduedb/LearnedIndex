...
def rr_gauss():
    return random.gauss(5.0, 2.0)

def rr_gamma():
    return random.gammavariate(3.0, 2.0)
...
class RectangleGen(object):
    ...
    def rect(self, size=10.0):
        x,y,w,h = rr_gauss(),rr_gauss(),random.uniform(0.0,size), random.uniform(0.0,size)
        r = Rect(x,y,x+w,y+h)
    ...
    def pointsVertex(self, r):
        return [(r.x, r.y), (r.x, r.yy), (r.xx, r.y), (r.xx, r.yy)]

    def pointsOnBoundary(self, r):
        pts = []
        pts.append((random.uniform(r.x, r.xx), r.y))
        pts.append((random.uniform(r.x, r.xx), r.yy))
        pts.append((r.x, random.uniform(r.y, r.yy)))
        pts.append((r.xx, random.uniform(r.y, r.yy)))
        return pts
